...
class Breadcrumb extends Component {
    ...
    render() {
        ...
        prevProducts = products.filter(item => item.id < parseInt(productId) && item.lgPictures);
        ...
        nextProducts = products.filter( item => item.id > parseInt(productId) && item.lgPictures );    
            ...
            { type === "normal" ?
                <div className={ container }>
                    <ol className="breadcrumb">
                        <li className="breadcrumb-item"><Link to={ `${process.env.PUBLIC_URL}/demo-${demo}` }>Home</Link></li>
                        {   path.map( item => (
                                <li className="breadcrumb-item" key={ item[0] }><Link to={ `${process.env.PUBLIC_URL}/${ item[1] }` }>{ item[0] }</Link></li>
                        )) }
                ...
                type === "product" ?
                    ...
                    <ol className="breadcrumb">
                        <li className="breadcrumb-item"><Link to={ `${process.env.PUBLIC_URL}/demo-${demo}` }>Home</Link></li>
                        {   path.map( (item, index) => (
                                <li className="breadcrumb-item" key={ index }><Link to={ `${process.env.PUBLIC_URL}/${ item[1] }/default/27` }>{ item[0] }</Link></li>
                        )) }
                        <li className="breadcrumb-item active" aria-current="page">{title}</li>
                    </ol>
